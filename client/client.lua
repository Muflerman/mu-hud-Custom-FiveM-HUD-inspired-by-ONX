...
-- Nuevas variables para controlar visibilidad
local inMulticharacter = false
...
-- Eventos para detectar spawn selector
...
CreateThread(function()
    while true do
        ...
        if not shouldShowHUD() then
            if vehicleHUDActive or playerHUDActive then
                hideAllHUD()
            end
            Wait(1000)
            goto continue
...
RegisterNetEvent("hud:client:LoadMap", function()
    ...
    SetMinimapComponentPosition('minimap', 'L', 'B', 0.0 + minimapOffset, -0.049, 0.1770, 0.183)
    SetMinimapComponentPosition("minimap_mask", "L", "B", 0.0 + minimapOffset, 0.0, 0.138, 0.20)
